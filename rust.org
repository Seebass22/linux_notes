...
** option enum
   ...
   - use =as_deref= to return option with reference to value using [[https://doc.rust-lang.org/std/ops/trait.Deref.html][Deref]]
     - turns =Option<String>= to =Option<&str>= (and not &String like as_ref would do)
   https://stackoverflow.com/a/76396693
*** =and_then=
    use to chain fallibile operations that return none
    #+begin_src rust
      if let Some(item) = grid.get(*y).and_then(|row| row.get(*x)) {
          item
      }
    #+end_src
*** =is_some_and=
    returns true if some and matches predicate
    #+begin_src rust
      if grid[y].get(x2).is_some_and(|x| x.is_ascii_digit()) {
          grid[y][x2]
      }
    #+end_src
*** =map=
    turn an =Option<T>= into an =Option<U>=
...
* let else
  bind variables in surrounding scope or return/continue/etc
  #+begin_src rust
    for targeted_enemy in query.iter() {
        let Some(entity) = targeted_enemy.0 else {
            continue;
        };
        // do something with entity
    }

    // works for slice destructuring too ofc
    let winning_and_own_numbers: Vec<Vec<&str>> = line
        .split('|')
        .map(|s| s.trim())
        .map(|s| s.split_whitespace().collect())
        .collect();
    let [winning_numbers, own_numbers] = &winning_and_own_numbers[..] else {
        unreachable!();
    };
 #+end_src
...
*** =split_once=
    splits a &str into two, returning =Option<(&str, &str)>=
    #+begin_src rust
...
#+begin_src rust
  let numbers = [4, 2, 5, 1, 6];
  // use & to dereference references, like destructuring
  // -> number: u32
  for &number in numbers.iter() {
      if number == 1 {
      }
  }
  // or dereference them yourself
  // -> number: &u32
  for number in numbers.iter() {
      if *number == 1 {
      }
  ...
  fn replace_joker(&self) -> Hand {
      ...
      // dereference double char reference
